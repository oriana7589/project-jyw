...
const LazyImagen = ({ codigoArticulo }) => {
    const [urlImagen, setUrlImagen] = useState(imagenNoDisponible);

    const fetchImagen = async () => {
        try {
            console.log('codigoArticulo lazy', codigoArticulo)
          const urlGetRequest = encodeURIComponent(`\\\\10.10.0.25\\fotos\\${codigoArticulo}-1.jpg`);
    ...
    return (
        <LazyLoad  offset={100}>
          <img src={urlImagen} alt={`Imagen de ${codigoArticulo}`} style={{ width: "100%", height: "100%", margin: "0.0rem" }} />
        </LazyLoad>
      );
    };    
